...
export const RageOfDatesModal: FC<RageOfDatesModalProps> = ({
  onClose,
  open,
  setEnd,
  setStart,
}: RageOfDatesModalProps) => {
  ...
  function createDatesArray(setDates: (val: Date[]) => void, start = true) {
    ...
    for (let i = creationDate.getFullYear(); i <= now.getFullYear(); i++) {
      console.log(i);
      for (
        let s = creationDate.getMonth();
        s <= (i === now.getFullYear() ? now.getMonth() : 12);
        s++
      ) {
    ...
    <>
      {open && (
        <Modal onClose={onClose}>
          <H3 center modalTitle style={{ marginTop: '0px' }}>
            Период операций
          </H3>
          <Select
          ...
          {selectType === 'Диапазон' && (
            <Rages>
              <Select
                ...
                defaultDesc={
                  listItemDateStart ? getMonth(listItemDateStart) : getMonth(startMonths[0])
                }
              ...
              <Select
                ...
                defaultDesc={
                  listItemDateEnd
                    ? getMonth(listItemDateEnd)
                    : getMonth(endMonths[endMonths.length - 1])
                }
