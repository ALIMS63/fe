...
export const Notifications = () => {
  ...
  function getNotifies(load = true) {
    if (hubConnection) {
      setLoading(load);
      hubConnection
        .invoke('GetInAppNotifications', [activeFilter === 'active' ? 0 : 1], 0, 100)
        .then((res) => {
  ...
  function onMore() {
    if (notifies && notifies.length <= allNotifies.length) {
      ...
      if (items.length) {
        setNotifies(
          [...notifies, ...items].sort((x: any, y: any) => {
        ...
        <Notifies.NotificationsMap>
          <Table.Table>
            ...
            {loading ? (
              ...
              <>
                {notifies.length === 0 ? (
                  <NotItems text="Не имеется уведомлений" />
                ) : (
                  <>
                    {notifies.map((notify: any, idx: number) => (
                      <Notifies.NotificationItem key={idx} newItem={notify.new}>
                        ...
                        <Table.Item item>
                          {notify.message}
                          {link(notify.notificationKind) &&
                            notify.link != '' &&
                            notify.link != '0' && (
                              <Table.LinkButton href={createLink(notify.link)}>
                                Перейти к обмену
                              </Table.LinkButton>
                            )}
                        </Table.Item>
                        <Notifies.DoneNotification
                          disabled={activeFilter === 'active'}
                          onClick={() => onNotify(notify.safeId)}
                        />
